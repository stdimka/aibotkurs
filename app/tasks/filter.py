import hashlib
from celery import shared_task

from app.redis_sync import get_sync_redis
from app.services.keyword_service import get_all_keywords
from app.utils.logging import get_logger

logger = get_logger(__name__)

FILTERED_TTL = 60 * 60 * 2      # 2 часа (увеличил немного)
DUP_TTL = 7 * 24 * 60 * 60     # 1 неделя для дедупликации


@shared_task(bind=True, name="filter_posts", max_retries=2)
def filter_posts_task(self, previous_results: list | None = None):
    """
    Задача фильтрации сырых новостей.
    Берёт новости из news:raw:*, фильтрует по ключевым словам и дедупликации,
    сохраняет отфильтрованные в news:filtered:* с изображением.
    """
    redis = get_sync_redis()

    try:
        keys = redis.keys("news:raw:*")
        if not keys:
            logger.info("Сырых новостей нет")
            return 0

        keywords = get_all_keywords(redis)
        keywords = set(kw.lower() for kw in keywords)

        processed = 0

        for key in keys:
            raw_data = redis.hgetall(key)
            if not raw_data:
                continue

            # Декодируем данные
            data = {
                (k.decode("utf-8") if isinstance(k, bytes) else k):
                (v.decode("utf-8") if isinstance(v, bytes) else v)
                for k, v in raw_data.items()
            }

            title = data.get("title", "")
            summary = data.get("summary", "")
            source = data.get("source", "unknown")
            published_at = data.get("published_at")
            image_url = data.get("image") or data.get("image_url") or ""

            content = f"{title} {summary}".lower()

            logger.info(f"[{source}] Пост: {title[:60]}..., проверяем на ключевые слова")

            # Фильтр по ключевым словам
            if keywords and not any(kw in content for kw in keywords):
                logger.info(f"[{source}] НЕ прошёл фильтр по ключевым словам")
                continue

            logger.info(f"[{source}] Прошёл фильтр по ключевым словам")

            # Дедупликация
            hash_digest = hashlib.md5(content.encode()).hexdigest()[:16]
            dup_key = f"news:dup:{hash_digest}"

            if redis.exists(dup_key):
                logger.debug(f"[{source}] Пропущено, дубликат")
                continue

            # Сохраняем отфильтрованную новость С ИЗОБРАЖЕНИЕМ
            filtered_key = f"news:filtered:{source}:{hash_digest}"

            redis.hset(
                filtered_key,
                mapping={
                    "title": title,
                    "url": data.get("url", ""),
                    "summary": summary,
                    "source": source,
                    "published_at": published_at or "",
                    "image": image_url,                    # ← КРИТИЧНО: сохраняем картинку
                }
            )

            redis.expire(filtered_key, FILTERED_TTL)

            # Отмечаем как обработанный (дубликат)
            redis.hset(
                dup_key,
                mapping={
                    "hash": hash_digest,
                    "title": title,
                    "source": source,
                }
            )
            redis.expire(dup_key, DUP_TTL)

            processed += 1
            logger.info(f"[{source}] Сохранена фильтрованная новость с изображением: {'✅' if image_url else '❌'}")

        logger.info(f"Отфильтровано и сохранено: {processed}")
        return processed

    except Exception as e:
        logger.error("Критическая ошибка в задаче фильтрации", exc_info=True)
        raise self.retry(exc=e, countdown=30)
